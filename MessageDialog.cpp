...
void MessageDialog :: ok( void )
{
    ...
    dynamic_cast<GLWorldViewer*>( parent() )->startStopTimer();
}
void MessageDialog :: exit( void )
{
    saveRecords( "ball_game_records.pw" );
    dynamic_cast<QWidget*>( parent()->parent()->parent() )->close();
}
...
void MessageDialog :: setEndMessage( RECORD * record )
{
    ...
    if( local_id != -1 )
    {
        ...
        for ( size_t id = dates.size() - 1; id > static_cast<size_t>( local_id ); id-- )
            dates[ id ]->setText( dates[ id - 1 ]->text() );
        for ( size_t id = names.size() - 1; id > static_cast<size_t>( local_id ); id-- )
            names[ id ]->setText( names[ id - 1 ]->text() );
        for ( size_t id = scores.size() - 1; id > static_cast<size_t>( local_id ); id-- )
            scores[ id ]->setText( scores[ id - 1 ]->text() );

        names[ static_cast<size_t>( local_id ) ]->setEnabled( true );
        //
        QTime now = QTime :: currentTime();
        QDate today = QDate :: currentDate();
        char str_auxiliar[100];
        sprintf( str_auxiliar, "%d/%d/%d-%d:%d", today.day(), today.month(), today.year(), now.hour(), now.minute() );
        dates[ static_cast<size_t>( local_id ) ]->setText( str_auxiliar );
        //
        sprintf( str_auxiliar, "<h3> %d Balls </h3>", dynamic_cast<GLWorldViewer*>( parent() )->getBallsCount() );
        scores[ static_cast<size_t>( local_id ) ]->setText( str_auxiliar );
    }
    //APENAS ACABOU
    else {
...
std :: vector< std :: string > MessageDialog :: returnSplitVector( std :: string line, std :: string separator )
{
    ...
    for ( size_t id = 0; id < line.size(); id++) {
        ...
        for ( size_t id_sep = 0; id_sep < separator_size; id_sep++)
        {
